...
# Validate and fix mod archive layouts for KOTOR.
...
class StarWarsKotorGame(BasicGame, mobase.IPluginFileMapper):
    ...
    def _log_platform_once(self, force: bool = False) -> bool:
        ...
        try:
            ...
            logger.info(
                "[KOTOR] Steam detected:%s path:%s steam_root:%s"
                % (self.is_steam(), gd.absolutePath(), steam_root)
            )
            self._platform_logged = True
        except Exception as e:
            logger.info("[KOTOR] Platform logging failed: %s", e)
        return True
    ...
    def _warn_if_workshop_present(self, steam_root: str):
        if steam_root.lower() == "unknown":
            return
        workshop_path = Path(steam_root) / "steamapps" / "workshop" / "content" / "32370"
        try:
            if workshop_path.exists() and any(workshop_path.iterdir()):
                logger.warning("[KOTOR] Steam Workshop content detected")
                try:
                    QTimer.singleShot(
                        2000,
            ...
            logger.debug("[KOTOR] Workshop check failed: %s", e)
    ...
    def _init_custom_tabs(self, main_window: QMainWindow):
        ...
        if textures_index is None:
            self._texture_tab = KotorTextureTab(main_window, self._organizer, self)
        ...
        if patcher_index is None:
            self._patcher_tab = KotorPatcherTab(main_window, self._organizer, self)
